...
#
# JETTY_HOME
#   Home of the Jetty installation.
#
# SOLR_HOME
...
if [ -z "$JETTY_HOME" ]
then
  JETTY_HOME="/usr/local/vufind-solr"
fi
...
if [  -z "$JETTY_PID" ]
then
  JETTY_PID="$JETTY_RUN/vufind-solr.pid"
fi

#####################################################
# Find a location for the jetty console
#####################################################
JETTY_CONSOLE=${JETTY_HOME}/logs/jetty.log
if [  -z "$JETTY_CONSOLE" ]
then
  if [ -w /dev/console ]
  then
...
case "$ACTION" in
  start)
        ...
        echo "STARTED Solr `date`" >> $JETTY_CONSOLE
        echo "$RUN_CMD"
        nohup sh -c "exec $RUN_CMD >>$JETTY_CONSOLE 2>&1" &
        echo $! > $JETTY_PID
        echo "Solr running pid="`cat $JETTY_PID`
        ;;
